...
from app.schemas.seat_layout import SeatLayoutWithTemplatesCreate
...
def create_seat_layout_with_templates(db: Session, layout_in: SeatLayoutWithTemplatesCreate):
    try:
        layout_name = db.query(SeatLayout).filter(SeatLayout.layout_name == layout_in.layout_name).first()
        if layout_name:
...
def generate_default_seat_templates(layout_id: int , total_rows: int, total_columns: int, exclude_positions: Set[Tuple[int, int]] = None) -> List[SeatTemplate]:
